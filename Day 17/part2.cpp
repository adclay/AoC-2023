...
int part2(std::istream &fin) {
	...
	for (int n = 0; n != N; ++n) {
		for (int m = 0; m != M; ++m) {
			...
			for (int offset = 1, cost = 0; offset != 11 && n - offset != -1; ++offset) {
			...
			for (int offset = 1, cost = 0; offset != 11 && n + offset != N; ++offset) {
				cost += cost_map[n + offset][m];
				if (offset >= 4) {
					digraph[v].push_back(make_pair(vertex(n + offset, m, false), cost));
					digraph[v].push_back(make_pair(vertex(n + offset, m, false), cost));
			...
			// East-West
			get<2>(v) = false;
			for (int offset = 1, cost = 0; offset != 11 && m - offset != -1; ++offset) {
			...
			for (int offset = 1, cost = 0; offset != 11 && m + offset != M; ++offset) {
				cost += cost_map[n][m + offset];
				if (offset >= 4) {
					digraph[v].push_back(make_pair(vertex(n, m + offset, true), cost));
					digraph[v].push_back(make_pair(vertex(n, m + offset, true), cost));
